...
<link rel="stylesheet"
	href="/static_resources/system/team2/team2_css/team2.css">
<title>관리자시스템</title>
</head>
<body class="page-body">
				...
				<template>
					<div class="flex flex-100" style="border: 2px solid #00CDFF;">
						<div class="flex-wrap flex-100 flex flex-gap-10 flex-padding-10"
							...
							<div class="form-group flex-100">
								...
								<select id="group_comm" class="form-control"
									v-model="gr_comm_no" @change="getCommonList()">
									<option value="">0.전체</option>
									<option v-for="(item, index) in gr_comm_list" :key="index"
										:value="item.gr_comm_no">{{ index+1 }}. {{
										item.gr_comm_nm }}</option>
								</select> <label class="fix-width-50"
									style="text-align: center; margin-left: 200px;">항목명 </label> <input
									class="form-control" v-model="search_val" value=""
					...
					<div class="flex flex-100 flex-padding-10 flex-gap-10"
						...
						<button v-if="!showButton" type="button"
							class="btn btn-gold btn-icon" style="margin-left: 5px;"
							@click="popGrComm">
							항목구분관리 <i class="entypo-archive"></i>
						</button>
						<button v-if="!showButton" type="button"
							class="btn btn-blue btn-icon" style="margin-left: 5px;"
							@click="showInputRow">
							항목코드추가 <i class="fa fa-plus"></i>
						</button>
						<button v-if="!showButton" type="button"
							class="btn btn-blue btn-icon" style="margin-left: 5px;"
							@click="showUpdateColumn">
							항목코드수정 <i class="entypo-archive"></i>
						</button>
						<button v-if="showButton" type="button"
							class="btn btn-success btn-icon" style="margin-left: 5px;"
							@click="save">
							저 장 <i class="entypo-archive"></i>
						</button>
						<button v-if="showButton" type="button"
							class="btn btn-blue btn-icon" style="margin-left: 5px;"
							@click="cancle">
							취 소 <i class="entypo-archive"></i>
						</button>
						<button v-if="!showButton" type="button"
							class="btn btn-danger btn-icon " style="margin-left: 5px;"
							@click="commDelete">
							항목코드삭제 <i class="entypo-trash"></i>
						</button>
					</div>
					<table class="table table-bordered datatable dataTable"
						...
						<thead>
							<tr>
								<th style="width: 4%; background-color: #B9FFFF;"
									class="center hidden-xs nosort"><input type="checkbox"
									id="allCheck" @click="all_check(event.target)"></th>
								<th style="width: 10%; background-color: #B9FFFF;"
									class="center">항목코드</th>
								<th style="width: 15%; background-color: #B9FFFF;"
									class="center">항목명</th>
								<th style="width: 15%; background-color: #B9FFFF;"
									class="center">항목구분</th>
								<th style="width: 15%; background-color: #B9FFFF;"
									class="center">비고</th>
								<th style="width: 10%; background-color: #B9FFFF;"
									class="center">등록(수정)일</th>
							</tr>
						</thead>
						<tbody class="table-group-divider">
							<tr class="input-row" v-if="insert">
								<td class="center" style="color: blue;">추가</td>
								<td><input type="text" v-model="comm_no"
									placeholder="항목코드 입력" /></td>
								<td><input type="text" v-model="comm_nm"
									placeholder="항목명 입력" /></td>
								<td><select id="group_comm_sel" class="form-control"
									v-model="sel_gr_comm_no">
										<option v-for="item in gr_comm_list" :value="item.gr_comm_no">{{
											item.gr_comm_nm }}</option>
								</select></td>
							...
							<tr v-for="(item, index) in dataList" class="input-row">
								...
								<td class="center" v-if="item.update"><input type="text"
									v-model="item.comm_nm" /></td>
								<td class="center" v-else>{{ item.comm_nm }}</td>
								<td class="center">{{ item.gr_comm_nm }}</td>
								<td class="center" v-if="item.update"><input type="text"
									v-model="item.note" /></td>
	...
	<!-- 팝업 -->
	<div class="modal fade" id="pop_gr_comm" tabindex="-1" role="dialog"
		aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" style="width: 400px;">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal"
						@click="close" aria-hidden="true" id="btn_popClose">&times;</button>
							...
							<tbody>
								<tr v-for="(item, index) in dataList" class="input-row">
									...
									<td class="center"><input type="text"
										v-model="item.gr_comm_nm" /></td>
							</tbody>
						</table>
						<div style="text-align: center; margin-bottom: 5px;">
							<button type="button" class="btn btn-green"
								style="margin-left: 5px;" @click="grCommUpdate">변경사항저장</button>
						</div>
					</div>
				</div>
			</div>
